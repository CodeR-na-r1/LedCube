...
void loop()
{
  ...
  for (int8_t i = 0; i < NUM_LEDS; i++) {
    if (stateLeds[i]) {
      turnLed(i);  // Включаем, если светодиод надо включить (выключит прошлый светодиод и включит текущий)
    }
    else {
      turnLed(-1);  // Выключаем, если светодиод  не надо включать (выключит прошлый светодиод)
    }

    delay(1);  // Задержка теперь общая, каждый светодиод имеет свой квант времени, в который он либо горит, либо не горит. Теперь яркость всегда одинаковая
  }
